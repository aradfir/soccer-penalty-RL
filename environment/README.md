# FORKED FROM RoboCup Soccer Simulator Server 18.1.3
## Environment for penalty learning
Changes made from the original:
- Always run the penalty on the right side of the field and always start with the left team
- Remove check that stops repeating penalty kicker

Make sure to use the server.conf in configuration file! paste in ~/.rcssserver/server.conf
changes made to conf:
- set number of halfs and extra halfs to zero to start game in penalty mode
- set stamina of all players to large value (to help repeating kickers)
- increased number of penalty kicks needed to win game (to help train model)
##
